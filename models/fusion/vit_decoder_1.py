...
class decoder_fuser_1(nn.Module):
    def __init__(self, dim, num_heads, num_layers, drop_rate):
        super(decoder_fuser_1, self).__init__()
        model_list = []
        ...
        # self.conv = nn.Conv1d(5+1, 5+1, 1)

    def forward(self, x_frame, x_heatmap):
        # x_frame = self.pos_drop(x_frame+self.pos_embed)
        # x_comb = torch.cat((x_frame, x_heatmap.unsqueeze(1)), dim=1)

        for _layer in self.model:
            x_frame = _layer(x_frame, x_heatmap)
...
if __name__=='__main__':
    ...
    model = decoder_fuser_1(dim=512, num_heads=8, num_layers=3, drop_rate=0.)
    y = model(x,x)
    print(y)
