...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, in_channels, rel_type, num_heads=32, max_partition_size=(2,1), partition_size=(1, 1), attn_drop=0., rel=True):
        super(MultiHeadSelfAttention, self).__init__()
        self.in_channels = in_channels
        self.rel_type = rel_type
        self.num_heads = num_heads
        self.max_partition_size = max_partition_size # (32, 8)
        self.partition_size = partition_size # (16,8)
        self.scale = num_heads ** -0.5
        self.max_attn_area = max_partition_size[0] * max_partition_size[1]
        ...
        if self.rel:
            ...
            elif self.rel_type == 'type_3':
                self.relative_position_bias_table = nn.Parameter(torch.zeros((2 * max_partition_size[0] - 1), num_heads)) # (63, h)
                self.register_buffer("relative_position_index", get_relative_position_index_1d(max_partition_size[0]))
                trunc_normal_(self.relative_position_bias_table, std=.02)
                self.ones = torch.ones(max_partition_size[1], max_partition_size[1], num_heads)
            elif self.rel_type == 'type_2':
                self.relative_position_bias_table = nn.Parameter(
                    torch.zeros((2 * partition_size[0] - 1), partition_size[1], partition_size[1], num_heads))
                self.register_buffer("relative_position_index", get_relative_position_index_1d(partition_size[0]))
                trunc_normal_(self.relative_position_bias_table, std=.02)
            elif self.rel_type == 'type_4':
                self.relative_position_bias_table = nn.Parameter(
                    torch.zeros((2 * max_partition_size[0] - 1), max_partition_size[1], max_partition_size[1], num_heads)) # (63, 9, 9, h)
                self.register_buffer("relative_position_index", get_relative_position_index_1d(max_partition_size[0]))
                trunc_normal_(self.relative_position_bias_table, std=.02)
    ...
    def _get_relative_positional_bias(self):
        if self.rel_type == 'type_1':
            ...
            table = self.relative_position_bias_table[:(2*self.partition_size[0]-1),:] # (63, h) -> (x, h)
            # ori_index = self.relative_position_index
            index = self.relative_position_index[:self.partition_size[0], -self.partition_size[0]:].reshape(-1)# ()
            # index_view= index.view(-1)
            # ori_view = ori_index.view(-1)
            bias = table[index]
            relative_position_bias = bias.view(
                self.partition_size[0], self.partition_size[0], -1)
            ...
            table = self.relative_position_bias_table[:(2*self.partition_size[0]-1),...]
            index = self.relative_position_index[:self.partition_size[0], -self.partition_size[0]:].reshape(-1)
            relative_position_bias = table[index].view(
                self.partition_size[0], self.partition_size[0], self.partition_size[1], self.partition_size[1], -1)
            relative_position_bias = relative_position_bias.permute(0, 2, 1, 3, 4).contiguous().view(self.attn_area,
                                                                                                     self.attn_area, -1)
            relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()
...
class SkateFormerBlock(nn.Module):
    def __init__(self, in_channels, num_points=50, max_frames=256, kernel_size=7, num_heads=32,
        ...
        self.max_t_3_size = (max_frames//type_1_size[0], type_3_size[1]) # (256/t, L)
        self.max_t_4_size = (max_frames//type_1_size[0], type_4_size[1]) # (256/t, K)
        self.max_partition_size = [type_1_size, type_2_size, self.max_t_3_size, self.max_t_4_size]
        ...
        for i in range(len(self.partition_function)):
            attention.append(
                MultiHeadSelfAttention(in_channels=in_channels // (len(self.partition_function) * 2),
                                       ...
                                       max_partition_size=self.max_partition_size[i],
...
class SkateFormerBlockDS(nn.Module):
    def __init__(
            self, in_channels, out_channels, num_points=50, max_frames=256, kernel_size=7, downscale=False, num_heads=32,
            type_1_size=(1, 1), type_2_size=(1, 1), type_3_size=(1, 1), type_4_size=(1, 1),
            attn_drop=0., drop=0., rel=True, drop_path=0., mlp_ratio=4.,
...
class SkateFormerStage(nn.Module):
    def __init__(
            ...
            num_points=50, max_frames=256, kernel_size=7, num_heads=32,
            type_1_size=(1, 1), type_2_size=(1, 1), type_3_size=(1, 1), type_4_size=(1, 1),
            attn_drop=0., drop=0., rel=True, drop_path=0., mlp_ratio=4.,
            act_layer=nn.GELU, norm_layer_transformer=nn.LayerNorm):
...
class SkateFormer_DFEW_rgb1x1(nn.Module):
    def __init__(self, in_channels=5, depths=(2, 2, 2, 2), channels=(96, 192, 192, 192), max_frames=256,
                 embed_dim=64, num_people=2, num_frames=64, num_points=63, kernel_size=7, num_heads=32,
                 type_1_size=(1, 1), type_2_size=(1, 1), type_3_size=(1, 1), type_4_size=(1, 1),
        ...
        super(SkateFormer_DFEW_rgb1x1, self).__init__()
        ...
        if self.index_t:
            self.joint_person_embedding = nn.Parameter(torch.zeros(embed_dim, num_points))
            trunc_normal_(self.joint_person_embedding, std=.02)
        else:
            self.joint_person_temporal_embedding = nn.Parameter(
                torch.zeros(1, embed_dim, num_frames, num_points))
    ...
    def forward(self, input, index_t):
        ...
        output = input.permute(0, 3, 2, 1).contiguous()  # [B, C, T, V]
...
def SkateFormer_(**kwargs):
    return SkateFormer_DFEW_rgb1x1(
        depths=(2, 2, 2, 2),
...
if __name__=='__main__':
    input = torch.randn((1,63,128,2))
    traj_len = [60]
    idx = np.linspace(0, 128 - 1, 128).astype(np.int)
    index_t = 2 * idx.astype(np.float32) / 128 - 1
    # traj_len = torch.as_tensor(traj_len).reshape(-1, 1)
    index_t = torch.from_numpy(index_t).float()
    model = SkateFormer_()
    output = model(input, index_t)
    print(output)
